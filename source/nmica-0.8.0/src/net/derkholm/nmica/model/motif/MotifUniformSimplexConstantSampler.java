...
public class MotifUniformSimplexConstantSampler implements ContributionSampler, Serializable {
    ...
    public MotifUniformSimplexConstantSampler(double step) {
        this.step = step;
    }
    ...
    public PenalizedVariate sample(Object parent, ObjectMatrix1D uncles)
    {
        try {
            ...
            int vCol = (int) Math.floor(Math.random() * wmp.unmaskedColumns());
            for (int c = 0; c < wmp.unmaskedColumns(); ++c) {
                if (c == vCol) {
                    ...
                    try {
                        ...
                        if (Math.random() < 0.5) {
                            newDist = sampleDist(old);
                        } else {
                            newDist = sampleDistBack(old);
                        }
                        dists[c] = newDist;
                    } catch (Exception ex) {
    ...
    private Distribution sampleDist(Distribution dist) 
        throws Exception
    {
    ...
    private Distribution sampleDistBack(Distribution dist) 
        throws Exception
    {
        FiniteAlphabet alpha = (FiniteAlphabet) dist.getAlphabet();
        Distribution nudist = new NMSimpleDistribution(alpha);
